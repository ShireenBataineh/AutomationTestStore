...
//		continue2.click();
		...
		//Assertion: to ensure that the signup process works fine
		String actualAssert= "Your Account Has Been Created!";
		boolean actualAssertBool=driver.getPageSource().contains(actualAssert);
		Assert.assertEquals(actualAssertBool, true, " ");
	...
	public void Logout()
	{
				...
				boolean actualResult=driver.getPageSource().contains(text);
...
//		login.click();
		...
		//String text2="My Account";
		
		String name=FNames[num1];
//		boolean actualResult=driver.getPageSource().contains(name);
//		Assert.assertEquals(actualResult, true);
		String actualResult=driver.findElement(By.xpath("//div[@class='menu_text']")).getText();
		String expectedRresult="Welcome back "+name;
		Assert.assertEquals(actualResult, expectedRresult);
